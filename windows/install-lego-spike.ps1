# Install LEGO Education SPIKE (Windows)
# Installs or updates the latest LEGO Education SPIKE release via WinGet.
$ErrorActionPreference = 'Stop'

$packageName = "LEGO Education SPIKE"
$packageId = "LEGO.LEGOEducationSPIKE"
$startMenuShortcutName = "LEGO Education SPIKE"
$shortcutSearchPatterns = @("LEGO*SPIKE*.lnk", "SPIKE*.lnk")
$executableNames = @("LEGO Education SPIKE.exe", "SPIKE.exe")
$executableCandidatePaths = @(
    "$env:ProgramFiles\LEGO Education SPIKE\LEGO Education SPIKE.exe",
    "$env:ProgramFiles\SPIKE\SPIKE.exe",
    "${env:ProgramFiles(x86)}\LEGO Education SPIKE\LEGO Education SPIKE.exe",
    "${env:ProgramFiles(x86)}\SPIKE\SPIKE.exe",
    "$env:LOCALAPPDATA\Programs\LEGO Education SPIKE\LEGO Education SPIKE.exe",
    "$env:LOCALAPPDATA\Programs\SPIKE\SPIKE.exe"
)
$appDisplayNamePatterns = @("*LEGO*SPIKE*", "*SPIKE*")
