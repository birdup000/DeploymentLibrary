# Update MakeCode Arcade Web App Shortcut (Windows)
# Recreates all-users Desktop and Start Menu shortcuts for a configurable web app.
...
foreach ($shortcutPath in $shortcutPaths) {
    Write-Host "Refreshing shortcut: $shortcutPath"
    New-WebAppShortcut -ShortcutPath $shortcutPath -Url $appUrl -EdgePath $edgePath -DisplayName $shortcutName
}

Write-Host "$shortcutName shortcuts refreshed successfully for $appUrl."
