# Install Krita (Windows)
# Installs or updates the latest Krita release via WinGet.
$ErrorActionPreference = 'Stop'

$packageName = "Krita"
$packageId = "KDE.Krita"
$startMenuShortcutName = "Krita"
$shortcutSearchPatterns = @("Krita*.lnk")
$executableNames = @("krita.exe")
$executableCandidatePaths = @(
    "$env:ProgramFiles\Krita (x64)\bin\krita.exe",
    "$env:ProgramFiles\Krita\bin\krita.exe",
    "${env:ProgramFiles(x86)}\Krita (x64)\bin\krita.exe",
    "${env:ProgramFiles(x86)}\Krita\bin\krita.exe"
)
